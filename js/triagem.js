...
function criarOuObterAjuda(campo, id) {
  if (!campo) return null;

  const field = campo.closest('.field');
  if (!field) return null;

  let help = field.querySelector(`#${id}`);
...
function setCampoErro(campo, mensagem = '') {
  if (!campo) return;

  const field = campo.closest('.field');
  const help = criarOuObterAjuda(campo, `${campo.id}Ajuda`);

  campo.classList.toggle('input-invalid', Boolean(mensagem));
  if (field) field.classList.toggle('field-invalid', Boolean(mensagem));
  if (help) help.textContent = mensagem;
}
...
function dataNascimentoValida(dataFormatada) {
  if (!/^\d{2}\/\d{2}\/\d{4}$/.test(dataFormatada)) return false;
...
function validarCampoCPF(mostrarErro = true) {
  if (!cpfPaciente) return true;

  const cpf = cpfPaciente.value.trim();
  const completo = somenteNumeros(cpf).length === 11;
  const valido = completo && validarCPF(cpf);

  if (mostrarErro) {
    if (!cpf) setCampoErro(cpfPaciente, 'Informe o CPF do paciente.');
    else if (!completo) setCampoErro(cpfPaciente, 'O CPF precisa ter 11 dígitos.');
    else if (!valido) setCampoErro(cpfPaciente, 'CPF inválido. Confira os dígitos.');
    else setCampoErro(cpfPaciente, '');
  }

  return valido;
}

function validarCampoNascimento(mostrarErro = true) {
  if (!dataNascimento) return true;

  const nascimento = dataNascimento.value.trim();

  if (!nascimento) {
    if (mostrarErro) setCampoErro(dataNascimento, '');
    return true;
  }

  const completo = nascimento.length === 10;
  const valido = completo && dataNascimentoValida(nascimento);

  if (mostrarErro) {
    if (!completo) setCampoErro(dataNascimento, 'Use o formato DD/MM/AAAA.');
    else if (!valido) setCampoErro(dataNascimento, 'Data de nascimento inválida.');
    else setCampoErro(dataNascimento, '');
  }

  return valido;
}

function validarFormularioTriagem() {
  const nomeInput = document.getElementById('nomePaciente');
  const nomeValido = Boolean(nomeInput?.value.trim());
  const sintomasValido = Boolean(sintomasPaciente?.value.trim());
  const cpfValido = validarCampoCPF(true);
  const nascimentoValido = validarCampoNascimento(true);

  if (!nomeValido) setCampoErro(nomeInput, 'Informe o nome completo do paciente.');
  else setCampoErro(nomeInput, '');

  if (!sintomasValido) setCampoErro(sintomasPaciente, 'Descreva os sintomas principais.');
  else setCampoErro(sintomasPaciente, '');

  if (!nomeValido || !sintomasValido || !cpfValido || !nascimentoValido) {
    showToast('Revise os campos obrigatórios antes de salvar.');
    return false;
  }
...
function numeroCampo(id, fallback = 0) {
...
function avaliarClassificacaoPorSinais() {
  const dor = Number(painRange?.value || 0);
  const saturacao = numeroCampo('saturacaoPaciente', 98);
  const sistolica = numeroCampo('pressaoSistolica', 120);
  const diastolica = numeroCampo('pressaoDiastolica', 80);
  const fc = numeroCampo('frequenciaCardiaca', 82);
  const temp = numeroCampo('temperaturaPaciente', 36.7);
  const fr = numeroCampo('frequenciaRespiratoria', 18);
  const glasgow = numeroCampo('glasgowPaciente', 15);
  ...
  if (dor >= 2 || temp >= 37.2) return 'verde';

  return 'azul';
}

function atualizarClassificacaoAutomatica() {
  const risk = avaliarClassificacaoPorSinais();
  setRisk(risk, true);
}

if (cpfPaciente) {
  cpfPaciente.setAttribute('maxlength', '14');
  cpfPaciente.addEventListener('input', () => {
    cpfPaciente.value = aplicarMascaraCPF(cpfPaciente.value);
    validarCampoCPF(somenteNumeros(cpfPaciente.value).length === 11);
  });
  cpfPaciente.addEventListener('blur', () => validarCampoCPF(true));
}
...
if (dataNascimento) {
  ...
  dataNascimento.addEventListener('input', () => {
    ...
    if (idadePaciente) {
      idadePaciente.value = dataNascimento.value.length === 10 && dataNascimentoValida(dataNascimento.value)
        ? calcularIdade(dataNascimento.value)
        : '--';
    }

    validarCampoNascimento(dataNascimento.value.length === 10);
  });
  dataNascimento.addEventListener('blur', () => validarCampoNascimento(true));
}
...
[
  ...
  if (campo) campo.addEventListener('input', atualizarClassificacaoAutomatica);
  if (campo) campo.addEventListener('change', atualizarClassificacaoAutomatica);
});

if (painRange) {
  painRange.addEventListener('input', atualizarClassificacaoAutomatica);
}

atualizarClassificacaoAutomatica();
  ...
  document.querySelectorAll('#listaTodasTriagens .btn-ver-detalhe').forEach((botao) => {
    botao.onclick = () => {
      ...
      abrirDetalheTriagem({
        ...
        nascimento: linha.dataset.nascimento || 'Não informado',
...
const btnVoltarTriagem = document.getElementById('btnVoltarTriagem') || document.getElementById('btnVoltarDetalhe');
...
function preencherProntuario(dados) {
  ...
  const campoNascimentoProntuario = document.getElementById('prontuarioNascimento');
  if (campoNascimentoProntuario) campoNascimentoProntuario.textContent = dados.nascimento || 'Não informado';
...
function voltarParaTelaDeTriagem() {
  ...
  const areaTriagem = document.querySelector('.triage-page') || document.body;

  areaTriagem.scrollIntoView({
    behavior: 'smooth',
    block: 'start'
  });

  showToast('Você voltou para a tela de triagem.');
}

if (btnVoltarTriagem) {
  btnVoltarTriagem.addEventListener('click', voltarParaTelaDeTriagem);
}

if (btnSalvarProntuario) {
  btnSalvarProntuario.addEventListener('click', () => {
    showToast('Prontuário salvo com sucesso.');
    voltarParaTelaDeTriagem();
  });
}
...
function coletarDadosFormulario(nome, risk) {
  ...
  return {
    id: pacienteRecepcaoAtual?.id || ('TRI-' + Date.now().toString(36).toUpperCase()),
    paciente: nome,
    nome,
    classificacao: risk,
    tempo: '0 min',
    ...
    nascimento: dataNascimento?.value || 'Não informado',
...
function criarLinhaModal(dados) {
  ...
  linha.dataset.nascimento = dados.nascimento || 'Não informado';
...
if (saveTriageBtn) {
  saveTriageBtn.addEventListener('click', () => {
    if (!validarFormularioTriagem()) return;
    ...
    if (window.HospitalFlow) {
      HospitalFlow.pushUnique(HospitalFlow.STORE.doctorQueue, {
        ...dados,
        enviadoParaMedicoEm: new Date().toISOString(),
        status: 'Aguardando médico'
      });

      if (pacienteRecepcaoAtual?.id) {
        HospitalFlow.removeById(HospitalFlow.STORE.triageQueue, pacienteRecepcaoAtual.id);
      }
    }
